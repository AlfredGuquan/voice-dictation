...
- 转录速度优化（#1 音频降采样 + #2 LLM 跳过 heuristic + #4 连接预热）-- AudioRecorder 用 AVAudioConverter 输出 16k mono Int16 WAV（上传体积 13.4MB→1.1MB，-92%）；LLMCleanupService 加 `canSkipCleanup`（无填充词 + 无 3 连重复 + 转录文本未命中替换 key 时跳过 GPT-4o-mini）；新增 `NetworkClient` 共享 URLSession + app 启动和 hotkey-down 时 prewarm TLS。`Config.cleanupMode` 三档 always/auto/never，env `VD_CLEANUP_MODE` 优先。35s 中文 TTS 实测（n=5/配置）：baseline asr 21.3s + llm 4.5s = **25.7s**；+#1 asr 9.9s + llm 5.3s = **15.2s（-41%）**；+#1+#2(auto skip, 温启动) **6.8s（-73%）**；+#1+#2+#4(冷启动 + 预热) **9.7s（-62%）**；冷启动单独预热收益 ~2.6s。bench 脚本和 fixture 生成在 `eval/speedtest/`。[2026-05-12]
- 路径 A（OpenAI Whisper-1 分片并行）验证 → **不采用** -- 在 eval 框架内实现 `engines/whisper_chunked_{core,30s,always}.py`（VAD 切句 + 并行 + 拼接），跑了 N=5 重复采样和 LLM judge。结论：长音频平均提速仅 -22%（不是单次跑看到的 -45%，那是 OpenAI 服务端噪声），且 chunked 引入了 baseline 没有的边界质量退化（4 处分片首词专业术语错：LLM→11m / 整段→诊断 / 一个HTTP→1ghttp / 松开→分开；1 处接缝重复）。jiwer CER 看不出语义级问题，必须靠 LLM judge。-22% 提速换不回术语错误，对 coding 场景不划算。决策：放弃路径 A，长音频提速等 round 2 选型（WhisperKit 本地 / SenseVoice / Groq 本身就解决了线性增长 + 短音频固定耗时两个问题）。chunked engine 文件保留作为未来 round 2 横向对比的对照组。详细数据：`eval/reports/judge_chunked.md` + `eval/reports/by_engine.csv`。补录的 3 条长音频（041=67s/042=82s/043=121s）已入 `eval/recorder/recordings/`，ground truth 已加入 `eval/recorder/scripts.json` [2026-05-10]
